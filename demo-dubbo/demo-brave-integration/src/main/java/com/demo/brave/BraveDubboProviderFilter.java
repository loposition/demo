...
public class BraveDubboProviderFilter implements Filter {
  ...
  public BraveDubboProviderFilter() {
    this.handler = BeanUtils.getBean(DubboServerHandler.class);
    this.tracer = BeanUtils.getTracing().tracer();
    this.extractor = BeanUtils.getTracing().propagation().extractor(Map::get);
  }
  ...
  private void preHandle() {
    handler.handleReceive(extractor);
  }
}
